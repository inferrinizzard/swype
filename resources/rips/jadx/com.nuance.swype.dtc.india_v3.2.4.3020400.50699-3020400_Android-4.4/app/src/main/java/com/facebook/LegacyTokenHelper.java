...
public final class LegacyTokenHelper {
    ...
    public static boolean hasTokenInformation(Bundle bundle) {
        String token;
        return (bundle == null || (token = bundle.getString(TOKEN_KEY)) == null || token.length() == 0 || bundle.getLong(EXPIRATION_DATE_KEY, 0L) == 0) ? false : true;
    }
    ...
    private void deserializeKey(String key, Bundle bundle) throws JSONException {
        ...
        if (valueType.equals(TYPE_ENUM)) {
            try {
                Enum<?> enumValue = Enum.valueOf(Class.forName(json.getString(JSON_VALUE_ENUM_TYPE)), json.getString(JSON_VALUE));
