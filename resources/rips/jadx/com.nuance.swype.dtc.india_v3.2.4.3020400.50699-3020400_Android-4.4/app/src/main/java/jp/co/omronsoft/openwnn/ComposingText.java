...
public class ComposingText {
    ...
    public void debugout() {
        for (int i = 0; i < 3; i++) {
            new StringBuilder("ComposingText[").append(i).append("]");
            new StringBuilder("  cur = ").append(this.mCursor[i]);
            StringBuilder sb = new StringBuilder();
            Iterator<StrSegment> it = this.mStringLayer[i].iterator();
            while (it.hasNext()) {
                StrSegment ss = it.next();
                sb.append('(').append(ss.string).append(',');
                sb.append(ss.from).append(',');
                sb.append(ss.to).append(')');
            }
            new StringBuilder("  str = ").append((Object) sb);
        }
    }
            ...
            if (strUplayer.size() <= 0) {
                ...
                if (last.to < mod_from) {
                    ...
                    if (uplayer_mod_from >= 0) {
                        ...
                        mod_len = 1;
                        org_len = (uplayer_org_to - uplayer_mod_from) + 1;
                        mod_from = uplayer_mod_from;
                        layer = uplayer;
                    } else {
                        strUplayer.add(0, new StrSegment(toString(layer, mod_from, mod_to), mod_from, mod_to));
                        for (int i4 = 1; i4 < strUplayer.size(); i4++) {
                            StrSegment ss4 = strUplayer.get(i4);
                            ss4.from += diff;
                            ss4.to += diff;
                        }
                        mod_from = 0;
    ...
    public int setCursor(int layer, int pos) {
        ...
        if (layer == 0) {
            ...
            if (this.mCursor[1] > 0) {
                c = 0;
                i = this.mStringLayer[1].get(this.mCursor[1] - 1).to + 1;
            } else {
                c = 0;
            }
        }
