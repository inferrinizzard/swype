...
public class RomkanFullKatakana implements LetterConverter {
    private static final HashMap<String, String> mRomkanTable = new HashMap<String, String>() { // from class: jp.co.omronsoft.openwnn.JAJP.RomkanFullKatakana.1
    ...
    public static boolean convert(ComposingText text, HashMap<String, String> table) {
        ...
        while (start < 3) {
            ...
            String match = table.get(key.toString().toLowerCase(Locale.getDefault()));
            if (match != null) {
                if (upper) {
                ...
                if (Pattern.compile(".*[a-zA-Z]$").matcher(text.toString(1)).matches()) {
                    text.moveCursor(1, -1);
                }
                return true;
            }
